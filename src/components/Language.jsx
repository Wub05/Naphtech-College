export default function Language() {
  return (
    <>
      <section className="code-section bg-white py-16 lg:py-24" id="sghuiyq8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center scroll-reveal opacity-0 translate-y-12 transition-all duration-700 ease-out">
            {/* <!-- Image --> */}
            <div className="relative">
              <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                <img
                  src="https://assets.ls-assets.com/provider/istock/2208292063.jpg?w=1200"
                  alt="Language Learning Classroom"
                  className="w-full h-auto"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#1E3A8A]/30 to-transparent"></div>
              </div>

              {/* <!-- Floating Stats --> */}
              <div className="absolute -top-4 -right-4 bg-white rounded-2xl shadow-xl p-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-[#10B981] rounded-lg flex items-center justify-center">
                    <svg
                      className="w-5 h-5 text-white"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
                      ></path>
                    </svg>
                  </div>
                  <div>
                    <div className="text-lg font-bold text-[#1F2937]">
                      All Levels
                    </div>
                    <div className="text-sm text-[#6B7280]">Welcome</div>
                  </div>
                </div>
              </div>
            </div>

            {/* <!-- Content --> */}
            <div>
              <div className="inline-flex items-center gap-2 bg-[#3B82F6]/10 rounded-full px-4 py-2 mb-6">
                <span className="text-[#3B82F6] text-sm font-semibold">
                  Language &amp; Academic Support
                </span>
              </div>

              <h2
                className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#1F2937] mb-6 leading-tight"
                style="font-family: var(--font-family-heading);"
              >
                Master English &amp; Excel in
                <span className="text-[#1E3A8A]"> Your Studies</span>
              </h2>

              <p className="text-lg text-[#6B7280] mb-8 leading-relaxed">
                Beyond technical skills, we offer comprehensive language
                programs and academic support to ensure your success in all
                areas of your education.
              </p>

              {/* <!-- Features --> */}
              <div className="grid sm:grid-cols-2 gap-5 mb-8">
                <div className="flex items-start gap-4 group">
                  <div className="flex-shrink-0 w-12 h-12 bg-[#F59E0B] rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                    <svg
                      className="w-6 h-6 text-white"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129"
                      ></path>
                    </svg>
                  </div>
                  <div>
                    <h3 className="text-lg font-semibold text-[#1F2937] mb-1">
                      Spoken English Programs
                    </h3>
                    <p className="text-[#6B7280] text-sm">
                      Build confidence in everyday and professional
                      conversations
                    </p>
                  </div>
                </div>

                <div className="flex items-start gap-4 group">
                  <div className="flex-shrink-0 w-12 h-12 bg-[#8B5CF6] rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                    <svg
                      className="w-6 h-6 text-white"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"
                      ></path>
                    </svg>
                  </div>
                  <div>
                    <h3 className="text-lg font-semibold text-[#1F2937] mb-1">
                      Study Guides &amp; Resources
                    </h3>
                    <p className="text-[#6B7280] text-sm">
                      Comprehensive materials to support your learning journey
                    </p>
                  </div>
                </div>

                <div className="flex items-start gap-4 group">
                  <div className="flex-shrink-0 w-12 h-12 bg-[#EC4899] rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                    <svg
                      className="w-6 h-6 text-white"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"
                      ></path>
                    </svg>
                  </div>
                  <div>
                    <h3 className="text-lg font-semibold text-[#1F2937] mb-1">
                      Group Practice Sessions
                    </h3>
                    <p className="text-[#6B7280] text-sm">
                      Learn together with fellow students in interactive
                      sessions
                    </p>
                  </div>
                </div>

                <div className="flex items-start gap-4 group">
                  <div className="flex-shrink-0 w-12 h-12 bg-[#06B6D4] rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                    <svg
                      className="w-6 h-6 text-white"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"
                      ></path>
                    </svg>
                  </div>
                  <div>
                    <h3 className="text-lg font-semibold text-[#1F2937] mb-1">
                      One-on-One Tutoring
                    </h3>
                    <p className="text-[#6B7280] text-sm">
                      Personalized support for your specific learning needs
                    </p>
                  </div>
                </div>
              </div>

              <a
                href="/programs/english-language"
                className="inline-flex items-center gap-2 bg-[#1E3A8A] hover:bg-[#1E40AF] text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300 hover:shadow-lg hover:shadow-[#1E3A8A]/25 hover:-translate-y-0.5"
              >
                Explore Language Programs
                <svg
                  className="w-5 h-5"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M17 8l4 4m0 0l-4 4m4-4H3"
                  ></path>
                </svg>
              </a>
            </div>
          </div>
        </div>
      </section>
    </>
  );
}
